...
const Dashboard = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [b, ex, inv] = await Promise.all([
      ...
      setLowStock((Array.isArray(inv) ? inv : []).filter((i: any) => 
        Number(i.available_quantity) <= Number(i.low_stock_threshold || 2)
      ));
      setExpenses((Array.isArray(ex) ? ex : []).filter((e: any) => e.date >= monthStart));
    } catch (error) {
  ...
  const calendar = useMemo(() => {
    ...
    return { cells, max, label: cursor.toLocaleDateString(t("locale") || "en-IN", { month: "long", year: "numeric" }) };
  }, [cursor, bookings, t]);
    ...
    <>
      ...
      <div className="rounded-xl border bg-card shadow-card overflow-hidden mb-8">
        ...
        <div className="grid grid-cols-7 text-center text-[10px] uppercase tracking-wider text-muted-foreground border-b bg-muted/30">
          {[t("sun"), t("mon"), t("tue"), t("wed"), t("thu"), t("fri"), t("sat")].map((d) => <div key={d} className="py-2">{d}</div>)}
        </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted/50 text-xs uppercase tracking-wider text-muted-foreground">
              <tr>
                ...
                <th className="text-left px-5 py-3">{t("eventDate")}</th>
            ...
            <tbody>
              {bookings.length === 0 && (
                <tr><td colSpan={6} className="text-center text-muted-foreground py-10">{t("noBookingsMatch")}</td></tr>
              )}
              {bookings.slice(0, 8).map((b) => (
                <tr key={b.id} className="border-t hover:bg-muted/30 cursor-pointer" onClick={() => setDrawerBooking(b)}>
                  ...
                  <td className="px-5 py-3"><Badge variant="outline" className={statusTone[b.status] || ""}>{t(b.status) || b.status}</Badge></td>
                  <td className="px-5 py-3"><Badge variant="outline" className={statusTone[b.payment_status] || ""}>{t(b.payment_status) || b.payment_status}</Badge></td>
          ...
          <div className="mt-5">
            {cardDrill?.key === "profit" ? (
              <div className="space-y-3 text-sm">
                <div className="flex justify-between border-b pb-2"><span>{t("revenue")} ({t("thisMonth")})</span><span className="text-success font-medium">{fmtINR(stats.monthlyRevenue)}</span></div>
                <div className="flex justify-between border-b pb-2"><span>{t("expenses")} ({t("thisMonth")})</span><span className="text-destructive font-medium">{fmtINR(stats.monthlyExpenses)}</span></div>
              ...
              <div className="text-center text-muted-foreground py-10">{t("noBookingsMatch")}</div>
            ) : (
              <div className="space-y-2">
